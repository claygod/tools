...
type RevolverChannel16Bit[T any] struct {
	mu       sync.Mutex
	list     [limit16bit]chan T
	shiftIn  uint16
	shiftOut uint16
	In       chan T
	Out      chan T
...
func NewRevolverChannel16Bit[T any](chCap int) *RevolverChannel16Bit[T] {
	var list [limit16bit]chan T
	list[0] = make(chan T, chCap)

	rCh := &RevolverChannel16Bit[T]{
		list:     list,
		shiftIn:  0,
		shiftOut: 0,
		In:       make(chan T),
...
func (r *RevolverChannel16Bit[T]) workerIn() {
	for {
		val, ok := <-r.In
		if !ok {
...
func (r *RevolverChannel16Bit[T]) wStop() {
	<-r.stopCh
...
func (r *RevolverChannel16Bit[T]) workerOut() {
	for {
		// is closed
...
func (r *RevolverChannel16Bit[T]) start() {
...
func (r *RevolverChannel16Bit[T]) Utilization() float64 {
	...
	return float64(delta) / float64(limit16bit) * 100
}
